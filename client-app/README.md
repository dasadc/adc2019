...
動作確認しているウェブブラウザは、Chromeであり、たまにFirefoxで確認もしている。

Edgeは（Chromium版Edgeも）、動作未確認である。
...
- "File Checker" 〜 "Help"の部分は、画面モードを切り替えるための**メニュー**。クリックすることで、画面モードが切り替わる。
- 時刻、Round、State、〜 User、"update status"の部分は、**ステータス・バー**である
    - 左端に、現在時刻が表示される
    - "Round"に、Roundカウンタ値が表示される。この値の意味は、999はテスト期間中、1は事前競技中、2は当日の本番競技中、3は当日本番競技終了後に当日参加（＝人力で解く）の採点・集計用である。[参考](../conmgr.md#schedule)
...
- ステータス・バーより下の領域に、メニューで選択した画面モードごとの表示が行われる。
	

<a name="file-checker"></a>
### File Checker
...
ログイン完了後は、メニューバーの下にあるステータス・バーに、`User: xxxxxx`のようにユーザー名が表示される。**(注意)** 表示が更新されるまで、数秒かかる場合がある
...
- 表が表示されないときは、"Refresh Q list"をクリックする。それでも何も表示されない場合は、まだ出題リストが作られていないときである。
- "Download zip"ボタンは、すべてのQファイルを、Zip形式で一括ダウンロードする。まだ出題リストが作られていないとき(表が表示されていないとき)は、ダウンロードできない
- 表の中の行ごとにある"Download"ボタンは、1つずつ、ファイルをダウンロードする
- 表の中の行ごとにある"View"ボタンは、ブラウザ上で表示する
...
- **(特別処理)** チーム名"ADC-0"(司会者用)は、"Quality point"と"Fastest point"の計算から除外される。"ADC-0"は運営側で出題した問題の模範解答例をViewerで表示させるために使う。
...
一般ユーザーの場合、表示されない。~~でも、なにかしら表示されているが、実際には何も操作できない。すべてエラーになる。~~
...
この画面には２つのモードが用意されていて、それぞれで、「問題データ作成のための機能」、「人力で問題を解くための機能」の２つの機能を提供している。
...
- TimekeeperのRoundでは、Roundカウンタ値を変更できる。この値の意味は、999はテスト期間中、1は事前競技中、2は当日の本番競技中、3は当日参加（＝人力で解く）の採点・集計用、である。[参考](../conmgr.md#schedule)。QデータとAデータは、Roundカウンタの値ごとに、個別に管理されているので、Roundカウンタを変更すると、Qデータ、Aデータ、スコアの表示が切り替わる。
