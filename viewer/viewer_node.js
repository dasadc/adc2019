...
function writepng(){
    // データを保存
    var visname = afile.slice(0,afile.indexOf(".txt")) + "_vis.png"
    canvas_saver().save(canvas, visname, function(){
        console.log("png created: " + visname);
    });
}
...
const canvas = createCanvas(boardW*gridSize+boardOrigX*2, boardH*gridSize+boardOrigY*2);
...
function doall(qfile, afile){
    fs.readFile(qfile, "utf-8", (err, data) => {
        if (err) throw err;
        parseQFile(data);
        ctx.clearRect(0, 0, canvas.width, canvas.height);

        fs.readFile(afile, "utf-8", (err, data) => {
            if (err) throw err;
            parseAFile(data);
